...
The protocol combines classical 3-party Diffie-Hellman (Ristretto255) with the post-quantum KEM ML-KEM-768 (CRYSTALS-Kyber) in an AND-model hybrid construction, layered on the OPAQUE password-authenticated key exchange framework.
...
**Files:** `rust/crates/opaque-client/tests/security_properties.rs`, `security_proptest.rs`
**Result:** 39/39 pass
...
Rust:       cargo test (libsodium-sys, proptest 1.x)
```
