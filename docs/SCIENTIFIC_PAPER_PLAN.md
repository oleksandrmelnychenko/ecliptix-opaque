...
- Обмеження: debug logging в dev-режимі; формальна верифікація виконана у символьній моделі (ProVerif), але не в обчислювальній (CryptoVerif)
...
- Обмеження запропонованого підходу:
  ...
  - Символьна верифікація (ProVerif 2.05) виконана для 5 властивостей; обчислювальна верифікація (CryptoVerif) — future work
  - Залежність від коректності libsodium/liboqs
- Напрямки подальших досліджень:
  - Обчислювальна верифікація у CryptoVerif (UC-security)
...
- Формальна верифікація проведена у ProVerif 2.05 (символьна модель Dolev–Yao): 5 властивостей, усі підтверджені; CryptoVerif (обчислювальна модель) — future work
