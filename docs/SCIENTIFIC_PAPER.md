...
Конструкція зберігає всі властивості OPAQUE (секретність паролю, forward secrecy, взаємну автентифікацію, стійкість до офлайн-перебору) та додає стійкість до квантових атак за AND-моделлю. Формальна верифікація: ProVerif 2.05 (5/5 запитів підтверджено, включаючи секретність при компрометації LTK сервера) та Tamarin Prover (8 лем, включаючи AND-model security). Емпірична валідація: 21/21 функціональних тестів (702 assertions), 22/23 тестів властивостей безпеки (1410 assertions). Протокол зберігає 3 повідомлення та 1.5 RTT класичного OPAQUE; overhead складає +2272 байти, зумовлений виключно артефактами ML-KEM-768. Обчислювальні витрати — 5.6 мкс на гібридний комбінатор; повний раунд ML-KEM-768 (54 мкс) швидший за 3-DH (134 мкс); 99.8% латентності припадає на Argon2id. Крос-платформне бенчмаркування: 586 мс (Apple M1 Pro ARM) та 203 мс (x86-64 PC) end-to-end автентифікація; серверна пропускна здатність 3010 KE2/с (M1).
...
Стаття організована таким чином. У розділі 2 наведено аналіз літературних джерел та існуючих рішень. Розділ 3 присвячено формальній постановці задачі. У розділі 4, що становить основну частину роботи, детально описано запропонований протокол Hybrid PQ-OPAQUE. Розділ 5 містить аналіз безпеки, включаючи формальну верифікацію у системах ProVerif та Tamarin Prover. У розділі 6 описано реалізацію та наведено результати систематичної експериментальної оцінки: крос-платформні мікро- та макробенчмарки (підрозд. 6.2), оцінку безпеки реалізації (підрозд. 6.3), емпіричну валідацію властивостей безпеки (підрозд. 6.4) та детерміністичні тестові вектори (підрозд. 6.5). Розділ 7 містить обговорення результатів. Висновки подано у розділі 8.
...
**Обчислювальна продуктивність.** Для оцінки обчислювальної складності проведено систематичне вимірювання на двох апаратних платформах:

- **Платформа A (Apple M1 Pro):** 10 ядер ARM, 16 ГБ RAM, macOS Darwin 25.2.0, AppleClang 17, Release (-O3), одне ядро. Дата: 2026-02-16.
- **Платформа B (Windows PC):** 32 ядра x86-64, 31.8 ГБ RAM, Windows, MSVC 19.43, Release. Дата: 2026-02-17.

Результати наведено у таблиці 6.2, де для кожної операції зазначено медіанне значення латентності.

**Таблиця 6.2** — Мікробенчмарки криптографічних примітивів (медіанне значення, мкс)

| Операція | Apple M1 Pro | Windows PC x86-64 | Примітка |
|:---|:---:|:---:|:---|
| **Операції на Ristretto255** | | | |
| Генерація ключової пари | 18.9 | 19.9 | libsodium, постійний час |
| Скалярне множення (одне DH) | 43.4 | 64.5 | Базова операція DH |
| Три операції 3-DH | 133.7 | 196.0 | 3 × DH |
| **ML-KEM-768 (FIPS 203)** | | | |
| KeyGen | 17.1 | 68.7 | Генерація ефемерної пари |
| Encapsulation | 18.3 | 71.5 | Серверна сторона |
| Decapsulation | 20.5 | 48.2 | Клієнтська сторона |
| Повний раунд (KeyGen + Encaps + Decaps) | 54.1 | 191.5 | Сумарно |
| **OPRF (Ristretto255-SHA512)** | | | |
| Blind | 64.9 | 86.9 | Засліплення паролю |
| Evaluate | 43.7 | 64.5 | Серверне обчислення |
| Finalize | 74.6 | 91.8 | Завершення клієнтом |
| **Виведення ключів** | | | |
| HKDF-Extract (HMAC-SHA-512) | 1.6 | — | Екстракція ключового матеріалу |
| HKDF-Expand (64 байти) | 11.7 | — | Розширення ключа |
| HMAC-SHA-512 (256 байт) | 1.9 | — | Обчислення MAC |
| **Симетрична криптографія** | | | |
| XSalsa20-Poly1305 encrypt (64 Б) | 0.5 | — | Шифрування конверта |
| XSalsa20-Poly1305 decrypt (64 Б) | 0.7 | — | Розшифрування конверта |
| **Гібридний комбінатор** | | | |
| PQ Hybrid Combiner (HKDF-Extract) | 5.6 | 10.0 | Комбінування DH + KEM |
| **Хешування паролю** | | | |
| Argon2id (MODERATE: 3 іт., 256 МБ) | 592 400 | 205 311 | **Домінує у загальному часі** |

Аналіз таблиці 6.2 демонструє кілька важливих результатів. По-перше, повний раунд ML-KEM-768 на Apple M1 (54.1 мкс) складає лише **40%** від трьох операцій Діффі--Геллмана на Ristretto255 (133.7 мкс), тобто постквантовий компонент є навіть швидшим за класичний обмін ключами на платформі ARM. На платформі x86-64 ML-KEM (191.5 мкс) є порівнянним із 3-DH (196.0 мкс). По-друге, сумарний час усіх криптографічних операцій (крім Argon2id) не перевищує 1 мс на жодній із платформ, тоді як Argon2id займає 592 мс (M1) або 205 мс (x86-64). Це означає, що постквантовий overhead складає **менше 0.02%** від загальної латентності автентифікації.

**Рисунок 6.1** — Розподіл обчислювального часу автентифікації клієнта

```
Загальний час автентифікації клієнта: ~586 мс (Apple M1 Pro)
┌────────────────────────────────────────────────────────────────────┐
│                                                                    │
...
│                                                                    │
│  3-DH (3×Ristretto255)  ████████████████████████████  133.7 мкс   │
│  OPRF Finalize           ████████████████             74.6 мкс    │
│  ML-KEM Decaps           ████████                     20.5 мкс    │
│  ML-KEM KeyGen           ███████                      17.1 мкс    │
│  HKDF-Extract+Expand     ███                          13.3 мкс    │
│  PQ Hybrid Combiner      ██                           5.6 мкс     │
│  MAC compute+verify      █                            ~3 мкс      │
│                                                                    │
│  PQ overhead: ~0.04 мс (0.007% від загального часу)               │
└────────────────────────────────────────────────────────────────────┘
```

**Таблиця 6.3** — Латентність фаз протоколу

| Фаза | Операція | Apple M1 Pro | Windows PC | Примітка |
|:---|:---|:---:|:---:|:---|
| **Реєстрація** | | | | |
| | create_registration_request | 0.108 мс | — | OPRF Blind |
| | create_registration_response | 0.163 мс | — | Серверний OPRF |
| | finalize_registration | 623.5 мс | — | Argon2id домінує |
| | **Повна реєстрація (end-to-end)** | **615.6 мс** | **227.4 мс** | |
| **Автентифікація** | | | | |
| | generate_ke1 (клієнт) | 0.131 мс | — | OPRF + DH + KEM KeyGen |
| | generate_ke2 (сервер) | 0.474 мс | 0.551 мс | 3-DH + KEM Encaps + HKDF + MAC |
| | generate_ke3 (клієнт) | 613.3 мс | — | Argon2id + OPRF + DH + KEM Decaps |
| | finish (сервер) | 0.07 мс | — | MAC verify |
| | **Повна автентифікація (end-to-end)** | **586.0 мс** | **202.6 мс** | |

Ключова функція розтягування Argon2id домінує у загальному часі автентифікації клієнта, складаючи 99.8% обчислювального навантаження. Постквантова складова (ML-KEM) додає ~0.04 мс, що є непомітним на тлі Argon2id. Ця характеристика є фундаментальною перевагою протоколу OPAQUE: обчислювальна вартість визначається параметрами захисту від перебору паролів (KSF), а не криптографічними примітивами обміну ключами.

Різниця між платформами (586 мс на M1 проти 203 мс на PC) повністю пояснюється параметрами Argon2id: на Windows PC із 32 ядрами та оптимізованими x86-інструкціями пам'яті Argon2id виконується у ~2.9 разів швидше (205 мс проти 592 мс), що компенсує повільніші ML-KEM операції (3.5× повільніше через відсутність ARM SIMD-оптимізацій у liboqs).

**Таблиця 6.4** — Серверна пропускна здатність

| Метрика | Apple M1 Pro | Windows PC | Примітка |
|:---|:---:|:---:|:---|
| Повна автентифікація (з Argon2id) | 1.6 авт./с | 4.4 авт./с | End-to-end обмежено KSF |
| Генерація KE2 (без Argon2id) | 3009.6 оп./с | 1813.7 оп./с | Чиста серверна крипто-ємність |
| Час одного KE2 | 0.332 мс | 0.551 мс | 3-DH + KEM Encaps + HKDF + MAC |

При серверному розгортанні з кешованими записами реєстрації операція Argon2id виконується виключно на стороні клієнта. Сервер виконує лише операції DH, KEM та MAC, що потребують 0.33–0.55 мс на один KE2. Виміряна серверна пропускна здатність на Apple M1 Pro становить ~3010 операцій генерації KE2 на секунду (одне ядро), що забезпечує високу масштабованість: при горизонтальному масштабуванні на 8-ядерному сервері — до ~24 000 автентифікацій на секунду.

**Таблиця 6.5** — Крос-платформне порівняння продуктивності

| Операція | Коефіцієнт (PC / M1) | Напрямок | Пояснення |
|:---|:---:|:---:|:---|
| ML-KEM-768 KeyGen | 4.02× | M1 швидше | ARM NEON SIMD-оптимізації |
| ML-KEM-768 повний раунд | 3.54× | M1 швидше | ARM-специфічні інструкції |
| Ristretto255 3-DH | 1.47× | M1 швидше | Оптимізований ARM64 код libsodium |
| Argon2id (MODERATE) | 2.89× | PC швидше | 32 ядра + x86 оптимізації пам'яті |
| Повна автентифікація | 2.89× | PC швидше | Домінування Argon2id |
| Серверний KE2 | 1.66× | M1 швидше | Чисті крипто-операції |

Крос-платформне порівняння демонструє принципову особливість архітектури протоколу: продуктивність окремих криптопримітивів може відрізнятися у 3–4 рази між платформами, проте загальна латентність автентифікації визначається виключно параметрами Argon2id. Для серверних розгортань, де Argon2id не виконується, платформа ARM (Apple M1) забезпечує на 66% вищу пропускну здатність завдяки оптимізованим реалізаціям криптопримітивів.
...
### 6.4 Емпірична валідація властивостей безпеки

Окрім формальної верифікації у ProVerif та Tamarin Prover (розділ 5), проведено систематичне емпіричне тестування властивостей безпеки реалізації. Тестування виконано за допомогою фреймворку Catch2 v3.4.0 на платформі Apple M1 Pro.

**Функціональні тести.** Повний набір функціональних тестів охоплює 21 тестовий сценарій із 702 перевірками (assertions). Результат: **21/21 тестів пройдено, 702/702 перевірок підтверджено.** Покриття включає: повний цикл реєстрації (ініціатор + відповідач), повний цикл автентифікації (KE1 → KE2 → KE3), операції ML-KEM-768, формування та розкриття конверта, операції OPRF, виведення ключів HKDF, обробку помилок та безпеку пам'яті.

**Тести властивостей безпеки.** Додатково розроблено 23 спеціалізовані тести властивостей безпеки із 1411 перевірками. Результат: **22/23 тестів пройдено, 1410/1411 перевірок підтверджено.** Єдиний тест, що потребує коригування, стосується неправильного очікування щодо інвалідації старих облікових даних при повторній реєстрації — це коректна поведінка протоколу, оскільки інвалідація є операцією серверної бази даних, а не криптографічною властивістю.

**Таблиця 6.6** — Результати тестування властивостей безпеки

| Категорія | Тест | Результат |
|:---|:---|:---:|
| **Секретність паролю** | Різні паролі → різні записи | ✅ |
| | Однаковий пароль → різні сесійні ключі | ✅ |
| **Ізоляція** | Різні account\_id → різні облікові дані | ✅ |
| | Різні сервери → несумісні облікові дані | ✅ |
| **Зв'язування транскрипту** | Модифікація полів KE1 → автентифікація не проходить | ✅ |
| | Модифікація полів KE2 → автентифікація не проходить | ✅ |
| **AND-модель безпеки** | Спільний секрет KEM впливає на PRK | ✅ |
| | Класичний IKM впливає на PRK | ✅ |
| **Узгодження ключів** | Клієнт і сервер виводять ідентичні ключі (50 ітерацій) | ✅ |
| **Безпека пам'яті** | Усі чутливі дані обнулено після завершення протоколу | ✅ |
| **Доменне розділення** | Різні мітки HKDF → різні ключі | ✅ |
| **Forward secrecy** | 100 сесій → 100 унікальних сесійних ключів | ✅ |
| | Відстань Геммінга ~50% (підтверджує випадковість) | ✅ |
| | Рівномірний розподіл байтів (критерій χ²) | ✅ |
| **Стійкість ML-KEM-768** | Невірний секретний ключ → implicit reject | ✅ |
| | Нульовий шифротекст → неспівпадіння секрету | ✅ |
| | Чутливість до зміни одного біта | ✅ |
| | 200 інкапсуляцій → 200 унікальних спільних секретів | ✅ |
| | 50 генерацій ключів → 50 унікальних ключових пар | ✅ |
| **Захист від replay-атак** | Повтор старого KE2 → відхилено | ✅ |
| | Повтор старого KE3 → відхилено | ✅ |
| **Унікальність ефемерних значень** | Кожне KE1 має унікальні nonce, EC-ключ, KEM-ключ | ✅ |

Емпіричні результати тестування повністю узгоджуються з теоретичним аналізом безпеки (розділ 5) та підтверджують коректність реалізації. Зокрема, тести AND-моделі безпеки (рядки 7–8 таблиці 6.6) демонструють, що як класичний (DH), так і постквантовий (KEM) компоненти вносять вклад у виведення сесійного ключа, підтверджуючи гібридну конструкцію комбінатора.

### 6.5 Детерміністичні тестові вектори

Для забезпечення незалежної відтворюваності та верифікації реалізації згенеровано набір детерміністичних тестових векторів протоколу Hybrid PQ-OPAQUE. Тестові вектори містять повний трасу виконання протоколу від входів до сесійних ключів.

**Параметри тестового вектора:**

| Параметр | Значення |
|:---|:---|
| Протокол | Hybrid-PQ-OPAQUE v1 |
| KEM | ML-KEM-768 (FIPS 203) |
| Група DH | Ristretto255 |
| Хеш | SHA-512 |
| MAC | HMAC-SHA-512 |
| AEAD | XChaCha20-Poly1305 |
| KSF | Argon2id |
| Комбінатор | HKDF-Extract(label ‖ transcript\_hash, dh1 ‖ dh2 ‖ dh3 ‖ kem\_ss) |

Тестовий вектор включає:

1. **Входи:** серверний seed (32 байти), серверна ключова пара (Ristretto255), пароль (`test_vector_password_v1`), ідентифікатор облікового запису.

2. **Фаза реєстрації:** RegistrationRequest (32 байти), RegistrationResponse (64 байти), RegistrationRecord (168 байт) — усі значення наведені у шістнадцятковому форматі.

3. **Фаза автентифікації:** KE1 (1272 байти, включає публічний ключ ML-KEM-768), KE2 (1376 байт, включає шифротекст ML-KEM-768), KE3 (64 байти, MAC).

4. **Верифікація:** `client_session_key == server_session_key` — підтверджено; `client_master_key == server_master_key` — підтверджено.

Розміри повідомлень у тестовому векторі точно відповідають теоретичному аналізу з таблиці 6.1, що підтверджує коректність формату серіалізації. Тестові вектори опубліковані у файлі `test_vectors/test_vectors.json` репозиторію та можуть використовуватися для валідації альтернативних реалізацій протоколу.
...
*Тестові вектори.* Для забезпечення незалежної валідації реалізації згенеровано набір детерміністичних тестових векторів (підрозд. 6.5), що містять повну трасу виконання протоколу. Проте, оскільки стандартизованих міжнародних тестових векторів для гібридного PQ-OPAQUE не існує, верифікація обмежена порівнянням із власною еталонною реалізацією. Публікація тестових векторів у відкритому доступі сприятиме незалежній валідації іншими дослідницькими групами.
...
4. **Кількісно оцінено накладні витрати гібридизації** на двох апаратних платформах (Apple M1 Pro ARM та x86-64 Windows PC): додаткові 2272 байт (+516%) до сумарного обсягу обміну при нульовому збільшенні кількості раундів (1.5 RTT). Повний раунд ML-KEM-768 на Apple M1 (54.1 мкс) складає лише 40% від трьох операцій Ristretto255 DH (133.7 мкс); гібридний комбінатор HKDF додає 5.6 мкс; загальна латентність автентифікації: 586 мс (Apple M1) та 203 мс (Windows PC), з яких 99.8% припадає на Argon2id; серверна пропускна здатність — 3010 операцій KE2/с (M1) та 1814 оп./с (PC) на одному ядрі.
...
Практичне значення отриманих результатів полягає у забезпеченні шляху поступової міграції систем автентифікації за паролем до постквантової безпеки. Гібридний підхід дозволяє розпочати впровадження постквантового захисту вже сьогодні, не втрачаючи класичної безпеки у разі виявлення вразливостей у нових постквантових схемах. Прийнятний рівень накладних витрат (2.3 КБ на автентифікацію, ~0.04 мс додаткових обчислень для ML-KEM) та відсутність додаткових раундів обміну роблять протокол придатним для промислового застосування у системах, де автентифікація за паролем є основним механізмом доступу. Крос-платформне тестування підтвердило стабільну роботу на ARM (Apple M1 Pro, 586 мс) та x86-64 (Windows PC, 203 мс), при цьому різниця у латентності повністю пояснюється параметрами Argon2id, а не постквантовими компонентами.

6. **Проведено систематичну емпіричну валідацію безпеки** реалізації: 21/21 функціональних тестів (702/702 assertions), 22/23 тестів властивостей безпеки (1410/1411 assertions), включаючи перевірки AND-моделі, forward secrecy, replay-захисту, ізоляції сесій, стійкості ML-KEM та очищення пам'яті. Згенеровано детерміністичні тестові вектори для незалежної валідації реалізації.

**Напрямки подальших досліджень.** Запропонована конструкція залишає відкритими кілька важливих напрямків. По-перше, OPRF-компонент протоколу, побудований на групі Ristretto255, не є постквантово стійким (§5.2.8). Розробка повністю постквантової OPRF на основі решіткових задач (ідеальних решіток або Module-LWE) дозволить усунути останню класичну залежність протоколу. По-друге, представлений аналіз безпеки ґрунтується на game-based аргументах; повне машинне доведення у обчислювальній моделі (наприклад, CryptoVerif [40]) забезпечило б більш строгі гарантії. По-третє, доцільно розширити крос-платформне бенчмаркування на ARM-сервери та IoT-пристрої для точнішого визначення меж застосовності протоколу. Нарешті, інтеграція запропонованого протоколу в IETF-стандарт OPAQUE [6] потребує розробки відповідного Internet-Draft з тестовими векторами та аналізом зворотної сумісності.
