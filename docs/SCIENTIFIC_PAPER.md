...
Показано, що запропонована конструкція зберігає всі фундаментальні властивості безпеки протоколу OPAQUE: секретність паролю (пароль не розкривається навіть при компрометації сервера), пряму секретність (forward secrecy), взаємну автентифікацію сторін та стійкість до офлайн-атак перебору за словником. Водночас протокол набуває стійкості до квантових атак за моделлю "AND" -- для успішного зламу противнику необхідно подолати одночасно і класичну, і постквантову складові. Формальна верифікація протоколу виконана у системі автоматичного доведення ProVerif 2.05 (5 запитів безпеки: секретність сеансового ключа та паролю за умови компрометації довготривалого ключа сервера, взаємна автентифікація та ін'єктивна відповідність — усі підтверджені). Додатково побудовано символьну модель у Tamarin Prover (8 лем безпеки, включаючи AND-model security та forward secrecy). Емпірична верифікація включає тести властивостей безпеки (session key independence, transcript binding, KEM contribution, replay resistance) та систематичне тестування на основі BDD-фреймворку Catch2 v3.4.0.
...
Модель Tamarin успішно скомпільована та завантажена (`Theory closed`). Лема `and_model_security` є унікальною серед формальних моделей PAKE-протоколів: вона явно моделює сценарій, у якому противник компрометує один з двох криптографічних примітивів (класичний DH або постквантовий KEM), та перевіряє, що сеансовий ключ залишається секретним за умови стійкості іншого примітиву.
