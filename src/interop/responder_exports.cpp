...
ECLIPTIX_OPAQUE_C_EXPORT int opaque_server_finish(const opaque_server_handle_t *server_handle,
    ...
    if (result == Result::Success) {
        ...
        OPAQUE_SERVER_LOG_HEX("session_key", session_key, std::min(key.size(), session_key_buffer_size));
        OPAQUE_SERVER_LOG_HEX("master_key", master_key_out, MASTER_KEY_LENGTH);
        OPAQUE_SERVER_LOG("SUCCESS: server finished");
    }
    return static_cast<int>(result);
}
...
ECLIPTIX_OPAQUE_C_EXPORT int opaque_server_create_with_keys(
    ...
    OPAQUE_SERVER_LOG_HEX("private_key", private_key, private_key_len);
