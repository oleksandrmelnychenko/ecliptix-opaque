...
namespace {
constexpr char kRelayLogPrefix[] = "[ECLIPTIX-OPAQUE-RELAY] ";
constexpr size_t kHexPreviewBytes = 8;
}

#define OPAQUE_RELAY_LOG(fmt, ...) fprintf(stderr, "%s" fmt "\n", kRelayLogPrefix __VA_OPT__(,) __VA_ARGS__)
#define OPAQUE_RELAY_LOG_HEX(label, data, len) do { \
    fprintf(stderr, "%s%s: ", kRelayLogPrefix, label); \
...
#define OPAQUE_RELAY_LOG(fmt, ...) ((void)0)
#define OPAQUE_RELAY_LOG_HEX(label, data, len) ((void)0)
...
} relay_state_handle_t;

typedef struct {
    ResponderKeyPair *keypair;
} relay_keypair_handle_t;

ECLIPTIX_OPAQUE_C_EXPORT int opaque_relay_keypair_generate(relay_keypair_handle_t **handle) {
    OPAQUE_RELAY_LOG("=== opaque_relay_keypair_generate ===");
    if (!handle) {
        OPAQUE_RELAY_LOG("ERROR: InvalidInput - handle is null");
        return static_cast<int>(Result::InvalidInput);
    }
    try {
        if (!crypto::init()) {
            OPAQUE_RELAY_LOG("ERROR: crypto::init() failed");
        ...
        if (Result result = ResponderKeyPair::generate(*keypair); result != Result::Success) {
            OPAQUE_RELAY_LOG("ERROR: keypair generate failed: %d", static_cast<int>(result));
            delete keypair;
            return static_cast<int>(result);
        }
        const auto keypair_handle = new relay_keypair_handle_t;
        keypair_handle->keypair = keypair;
        *handle = keypair_handle;
        OPAQUE_RELAY_LOG("SUCCESS: keypair generated at %p", (void*)*handle);
...
ECLIPTIX_OPAQUE_C_EXPORT void opaque_relay_keypair_destroy(relay_keypair_handle_t *handle) {
    if (handle) {
...
ECLIPTIX_OPAQUE_C_EXPORT int opaque_relay_keypair_get_public_key(relay_keypair_handle_t *handle,
...
ECLIPTIX_OPAQUE_C_EXPORT int opaque_relay_create(relay_keypair_handle_t *keypair_handle,
                         opaque_relay_handle_t **handle) {
...
ECLIPTIX_OPAQUE_C_EXPORT int opaque_relay_state_create(relay_state_handle_t **handle) {
    if (!handle) {
        return static_cast<int>(Result::InvalidInput);
    }
    try {
        const auto state = new ResponderState();
        const auto state_handle = new relay_state_handle_t;
...
ECLIPTIX_OPAQUE_C_EXPORT void opaque_relay_state_destroy(const relay_state_handle_t *handle) {
    if (handle) {
        delete handle->state;
        delete handle;
    }
...
ECLIPTIX_OPAQUE_C_EXPORT int opaque_relay_create_registration_response(const opaque_relay_handle_t *relay_handle,
    ...
    OPAQUE_RELAY_LOG("=== opaque_relay_create_registration_response ===");
    OPAQUE_RELAY_LOG("relay_handle=%p, request_length=%zu (expected=%zu)",
    ...
    if (!relay_handle || !relay_handle->server || !request_data ||
        ...
        OPAQUE_RELAY_LOG("ERROR: InvalidInput");
        return static_cast<int>(Result::InvalidInput);
    }
    ...
    if (result == Result::Success) {
        ...
        OPAQUE_RELAY_LOG_HEX("response_data", response_data, REGISTRATION_RESPONSE_LENGTH);
        OPAQUE_RELAY_LOG("SUCCESS: registration response created");
    }
    return static_cast<int>(result);
}

ECLIPTIX_OPAQUE_C_EXPORT int opaque_relay_build_credentials(const uint8_t *registration_record, size_t record_length,
                                    uint8_t *credentials_out, size_t credentials_out_length) {
    OPAQUE_RELAY_LOG("=== opaque_relay_build_credentials ===");
    const size_t record_expected = REGISTRATION_RECORD_LENGTH;
    const size_t credentials_expected = RESPONDER_CREDENTIALS_LENGTH;
    OPAQUE_RELAY_LOG("record_length=%zu (expected=%zu), credentials_out_length=%zu (expected=%zu)",
    ...
    OPAQUE_RELAY_LOG_HEX("registration_record", registration_record, record_length);
    ResponderCredentials credentials;
    Result result = build_credentials(registration_record, record_length, credentials);
    if (result != Result::Success) {
        OPAQUE_RELAY_LOG("ERROR: build_credentials failed: %d", static_cast<int>(result));
        return static_cast<int>(result);
    }
    ...
    if (write_result != Result::Success) {
        OPAQUE_RELAY_LOG("ERROR: write_registration_record failed: %d", static_cast<int>(write_result));
        return static_cast<int>(write_result);
    }
    OPAQUE_RELAY_LOG_HEX("credentials_out", credentials_out, credentials_expected);
    OPAQUE_RELAY_LOG("SUCCESS: credentials built");
    return static_cast<int>(Result::Success);
}

ECLIPTIX_OPAQUE_C_EXPORT int opaque_relay_generate_ke2(const opaque_relay_handle_t *relay_handle,
                               ...
                               const relay_state_handle_t *state_handle) {
    OPAQUE_RELAY_LOG("=== opaque_relay_generate_ke2 ===");
    OPAQUE_RELAY_LOG("relay_handle=%p, state_handle=%p", (void*)relay_handle, (void*)state_handle);
    OPAQUE_RELAY_LOG("ke1_length=%zu (expected=%zu), credentials_length=%zu (expected=%zu), ke2_buffer_size=%zu (expected=%zu)",
    ...
    OPAQUE_RELAY_LOG_HEX("ke1_data", ke1_data, ke1_length);
    OPAQUE_RELAY_LOG_HEX("credentials_data", credentials_data, credentials_length);
    ...
    if (parse_result != Result::Success) {
        OPAQUE_RELAY_LOG("ERROR: Invalid credentials record (%d)", static_cast<int>(parse_result));
    ...
    if (result == Result::Success) {
        ...
        if (write_result != Result::Success) {
            OPAQUE_RELAY_LOG("ERROR: write_ke2 failed (%d)", static_cast<int>(write_result));
            return static_cast<int>(write_result);
        }
        OPAQUE_RELAY_LOG_HEX("ke2_data", ke2_data, KE2_LENGTH);
        OPAQUE_RELAY_LOG("SUCCESS: KE2 generated");
    }
    return static_cast<int>(result);
}

ECLIPTIX_OPAQUE_C_EXPORT int opaque_relay_finish(const opaque_relay_handle_t *relay_handle,
    ...
    OPAQUE_RELAY_LOG("=== opaque_relay_finish ===");
    OPAQUE_RELAY_LOG("relay_handle=%p, state_handle=%p", (void*)relay_handle, (void*)state_handle);
    ...
    OPAQUE_RELAY_LOG_HEX("ke3_data", ke3_data, ke3_length);
    secure_bytes key;
...
ECLIPTIX_OPAQUE_C_EXPORT int opaque_relay_create_default(opaque_relay_handle_t **handle) {
...
ECLIPTIX_OPAQUE_C_EXPORT int opaque_relay_derive_keypair_from_seed(
...
ECLIPTIX_OPAQUE_C_EXPORT int opaque_relay_create_with_keys(
    const uint8_t *private_key, const size_t private_key_len,
    const uint8_t *public_key, const size_t public_key_len,
    opaque_relay_handle_t **handle) {
    OPAQUE_RELAY_LOG("=== opaque_relay_create_with_keys ===");
    OPAQUE_RELAY_LOG("private_key_len=%zu, public_key_len=%zu", private_key_len, public_key_len);
    if (!private_key || private_key_len != PRIVATE_KEY_LENGTH ||
        !public_key || public_key_len != PUBLIC_KEY_LENGTH || !handle) {
        OPAQUE_RELAY_LOG("ERROR: InvalidInput");
        return static_cast<int>(Result::InvalidInput);
    }
    OPAQUE_RELAY_LOG_HEX("public_key", public_key, public_key_len);
    if (!crypto::init()) {
        OPAQUE_RELAY_LOG("ERROR: crypto::init() failed");
        return static_cast<int>(Result::CryptoError);
    }
    if (Result key_result = crypto::validate_public_key(public_key, PUBLIC_KEY_LENGTH);
        key_result != Result::Success) {
        OPAQUE_RELAY_LOG("ERROR: Invalid public key (%d)", static_cast<int>(key_result));
        return static_cast<int>(key_result);
    }
    uint8_t derived_public_key[PUBLIC_KEY_LENGTH];
    if (crypto_scalarmult_ristretto255_base(derived_public_key, private_key) != 0) {
        OPAQUE_RELAY_LOG("ERROR: scalar mult failed");
        sodium_memzero(derived_public_key, sizeof(derived_public_key));
        return static_cast<int>(Result::CryptoError);
    }
    if (crypto_verify_32(derived_public_key, public_key) != 0) {
        OPAQUE_RELAY_LOG("ERROR: derived public key doesn't match provided public key");
        sodium_memzero(derived_public_key, sizeof(derived_public_key));
        return static_cast<int>(Result::InvalidPublicKey);
    }
    sodium_memzero(derived_public_key, sizeof(derived_public_key));
...
ECLIPTIX_OPAQUE_C_EXPORT const char *opaque_relay_get_version() {
    return OPAQUE_RELAY_VERSION;
}

ECLIPTIX_OPAQUE_C_EXPORT size_t opaque_relay_get_ke2_length() {
    return KE2_LENGTH;
}

ECLIPTIX_OPAQUE_C_EXPORT size_t opaque_relay_get_registration_record_length() {
    return REGISTRATION_RECORD_LENGTH;
}

ECLIPTIX_OPAQUE_C_EXPORT size_t opaque_relay_get_credentials_length() {
    return RESPONDER_CREDENTIALS_LENGTH;
}

ECLIPTIX_OPAQUE_C_EXPORT size_t opaque_relay_get_kem_ciphertext_length() {
