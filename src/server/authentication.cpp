...
Result generate_ke2_impl(const uint8_t* ke1_data, size_t ke1_length,
    ...
    const uint8_t* client_nonce = ke1_data;
    const uint8_t* client_public_key = ke1_data + NONCE_LENGTH;
    const uint8_t* credential_request = ke1_data + NONCE_LENGTH + PUBLIC_KEY_LENGTH;
    ...
    if (crypto_scalarmult_ristretto255(client_server_shared, server_private_key.data(),
                                      client_public_key) != 0) {
        return Result::CryptoError;
    }

    uint8_t server_client_shared[PUBLIC_KEY_LENGTH];
    if (crypto_scalarmult_ristretto255(server_client_shared, state.server_private_key.data(),
                                      server_public_key.data()) != 0) {
        return Result::CryptoError;
    }

    uint8_t server_client_ephemeral[PUBLIC_KEY_LENGTH];
    if (crypto_scalarmult_ristretto255(server_client_ephemeral, state.server_private_key.data(),
                                      client_public_key) != 0) {
        return Result::CryptoError;
    }
    ...
    std::copy(server_client_ephemeral, server_client_ephemeral + PUBLIC_KEY_LENGTH,
