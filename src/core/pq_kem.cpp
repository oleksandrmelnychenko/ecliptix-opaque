...
namespace ecliptix::security::opaque::pq {
    namespace kem {
        ...
        Result keypair_generate(uint8_t *public_key, uint8_t *secret_key) {
            ...
            const OQS_KEM *kem_ctx = get_kem_ctx();
            if (!kem_ctx) [[unlikely]] {
                log::msg("ERROR: Failed to create ML-KEM-768 context");
                return Result::CryptoError;
            }

            if (const OQS_STATUS status = OQS_KEM_keypair(kem_ctx, public_key, secret_key); status != OQS_SUCCESS) [[
                unlikely]] {
        ...
        Result encapsulate(
            ...
            if (const OQS_STATUS status = OQS_KEM_encaps(kem_ctx, ciphertext, shared_secret, public_key);
                status != OQS_SUCCESS) [[unlikely]] {
        ...
        Result decapsulate(
            ...
            if (const OQS_STATUS status = OQS_KEM_decaps(kem_ctx, shared_secret, ciphertext, secret_key);
                status != OQS_SUCCESS) [[unlikely]] {
                sodium_memzero(shared_secret, KEM_SHARED_SECRET_LENGTH);
    ...
    Result combine_key_material(
        ...
        const size_t classical_ikm_length,
        const uint8_t *pq_shared_secret,
        const size_t pq_ss_length,
        const uint8_t *transcript_hash,
        const size_t transcript_length,
        uint8_t *prk
    ) {
        log::section("PQ: Combine Classical + Post-Quantum Key Material");

        if (constexpr size_t CLASSICAL_IKM_LENGTH = 3 * PUBLIC_KEY_LENGTH;
            !classical_ikm || classical_ikm_length != CLASSICAL_IKM_LENGTH ||
        ...
        std::copy_n(
            ...
            labeled_transcript.begin() + labels::kPqCombinerContextLength
        );

        const Result result = crypto::key_derivation_extract(
