...
Result generate_ke3_impl(const uint8_t* ke2_data, size_t ke2_length,
    ...
    const uint8_t* server_mac = ke2_data + NONCE_LENGTH + PUBLIC_KEY_LENGTH + CREDENTIAL_RESPONSE_LENGTH;
    ...
    env.auth_tag.assign(envelope_data + NONCE_LENGTH, envelope_data + NONCE_LENGTH + MAC_LENGTH);

    uint8_t recovered_server_public_key[PUBLIC_KEY_LENGTH];
    uint8_t recovered_client_private_key[PRIVATE_KEY_LENGTH];

    result = envelope::open(randomized_pwd, sizeof(randomized_pwd), env,
    ...
    uint8_t client_server_shared[PUBLIC_KEY_LENGTH];
    if (crypto_scalarmult_ristretto255(client_server_shared, recovered_client_private_key,
                                      server_public_key) != 0) {
        return Result::CryptoError;
    }

    uint8_t server_client_shared[PUBLIC_KEY_LENGTH];
    if (crypto_scalarmult_ristretto255(server_client_shared, state.client_private_key.data(),
                                      recovered_server_public_key) != 0) {
        return Result::CryptoError;
    }

    uint8_t client_server_ephemeral[PUBLIC_KEY_LENGTH];
    if (crypto_scalarmult_ristretto255(client_server_ephemeral, state.client_private_key.data(),
                                      server_public_key) != 0) {
        return Result::CryptoError;
    }

    secure_bytes ikm(3 * PUBLIC_KEY_LENGTH);
    std::copy(client_server_shared, client_server_shared + PUBLIC_KEY_LENGTH,
             ikm.begin());
    std::copy(server_client_shared, server_client_shared + PUBLIC_KEY_LENGTH,
             ikm.begin() + PUBLIC_KEY_LENGTH);
    std::copy(client_server_ephemeral, client_server_ephemeral + PUBLIC_KEY_LENGTH,
