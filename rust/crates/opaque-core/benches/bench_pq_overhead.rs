...
// Ecliptix Security — PQ vs Classic OPAQUE Overhead Benchmark
// Licensed under the MIT License
//
// Isolates the cost of adding ML-KEM-768 to each AKE phase.
// Argon2id/KSF is intentionally excluded so the asymmetric crypto overhead
// is clearly visible. Classic = 4DH only; Hybrid = 4DH + ML-KEM-768.
...
// ── KE1: Initiator key generation ─────────────────────────────────────────────
// Classic : OPRF blind  +  DH ephemeral keygen
// Hybrid  : same        +  ML-KEM-768 keypair generation  (+~41 µs)
...
// ── KE2: Responder message (no KSF) ───────────────────────────────────────────
// Classic : OPRF eval  +  4DH  +  HKDF-Extract(128 B)  +  4×Expand  +  2×HMAC
// Hybrid  : same       +  KEM encaps  +  combine_key_material(160 B)  (+~37 µs)
...
fn bench_ke2_no_ksf(c: &mut Criterion) {
    ...
    // KEM keypair (initiator's from KE1)
    let mut kem_pk = vec![0u8; pq::KEM_PUBLIC_KEY_LENGTH];
    ...
    group.bench_function("classic", |b| {
        ...
        b.iter(|| {
            ...
            crypto::key_derivation_expand(&prk, labels::SESSION_KEY_INFO, &mut session_key).unwrap();
            crypto::key_derivation_expand(&prk, labels::MASTER_KEY_INFO, &mut master_key).unwrap();
            crypto::key_derivation_expand(&prk, labels::RESPONDER_MAC_INFO, &mut resp_mac_key).unwrap();
            crypto::key_derivation_expand(&prk, labels::INITIATOR_MAC_INFO, &mut init_mac_key).unwrap();
    ...
    group.bench_function("hybrid", |b| {
        ...
        b.iter(|| {
            ...
            crypto::key_derivation_expand(&prk, pq_labels::PQ_MASTER_KEY_INFO, &mut master_key).unwrap();
            crypto::key_derivation_expand(&prk, pq_labels::PQ_RESPONDER_MAC_INFO, &mut resp_mac_key).unwrap();
...
// ── KE3: Initiator finish (no KSF) ────────────────────────────────────────────
// Classic : OPRF finalize  +  4DH  +  HKDF-Extract(128 B)  +  4×Expand  +  HMAC-verify
// Hybrid  : same           +  ML-KEM-768 decaps  +  combine_key_material  (+~42 µs)
...
fn bench_ke3_no_ksf(c: &mut Criterion) {
    ...
    // KEM: initiator keypair, responder encapsulates
    let mut kem_pk = vec![0u8; pq::KEM_PUBLIC_KEY_LENGTH];
    let mut kem_sk = vec![0u8; pq::KEM_SECRET_KEY_LENGTH];
    pq_kem::keypair_generate(&mut kem_pk, &mut kem_sk).unwrap();
    let mut kem_ct = vec![0u8; pq::KEM_CIPHERTEXT_LENGTH];
...
// ── Full AKE: KE1 + KE2 + KE3 combined (no KSF) ──────────────────────────────
// Measures total round-trip crypto cost on both sides without Argon2id.
// Delta = KEM keygen + KEM encaps + KEM decaps + 2×combine vs 2×HKDF-Extract
...
fn bench_full_ake_no_ksf(c: &mut Criterion) {
    ...
    group.bench_function("classic", |b| {
        b.iter(|| {
            ...
            crypto::key_derivation_expand(&prk2, labels::SESSION_KEY_INFO, &mut session_key).unwrap();
            crypto::key_derivation_expand(&prk2, labels::MASTER_KEY_INFO, &mut master_key).unwrap();
            crypto::key_derivation_expand(&prk2, labels::RESPONDER_MAC_INFO, &mut resp_mac_key2).unwrap();
            crypto::key_derivation_expand(&prk2, labels::INITIATOR_MAC_INFO, &mut init_mac_key2).unwrap();
    ...
    group.bench_function("hybrid", |b| {
        b.iter(|| {
            ...
            let mut init_mac_key2 = [0u8; MAC_LENGTH];
            let mut init_mac2 = [0u8; MAC_LENGTH];
            crypto::key_derivation_expand(&prk2, pq_labels::PQ_SESSION_KEY_INFO, &mut session_key).unwrap();
            crypto::key_derivation_expand(&prk2, pq_labels::PQ_MASTER_KEY_INFO, &mut master_key).unwrap();
            crypto::key_derivation_expand(&prk2, pq_labels::PQ_RESPONDER_MAC_INFO, &mut resp_mac_key2).unwrap();
            crypto::key_derivation_expand(&prk2, pq_labels::PQ_INITIATOR_MAC_INFO, &mut init_mac_key2).unwrap();
            let _ = crypto::verify_hmac(&resp_mac_key2, &mac_input, &resp_mac);
            crypto::hmac_sha512(&init_mac_key2, &mac_input, &mut init_mac2).unwrap();
        })
    });
