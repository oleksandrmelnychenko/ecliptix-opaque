...
fn bench_oprf_evaluate(c: &mut Criterion) {
    ...
    let server_key = crypto::random_nonzero_scalar();
...
criterion_group!(oprf_benches, bench_oprf_blind, bench_oprf_evaluate, bench_oprf_finalize,);
...
criterion_main!(ristretto, mlkem, oprf_benches, hkdf, hmac, aead, argon2, pq_combiner);
