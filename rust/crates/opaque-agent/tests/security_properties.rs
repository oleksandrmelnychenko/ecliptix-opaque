...
fn authenticate(
    ...
    let mut s_sk = Vec::new();
    let mut s_mk = Vec::new();
    responder_finish(&ke3_bytes, &mut server_state, &mut s_sk, &mut s_mk).unwrap();

    let mut c_sk = Vec::new();
    let mut c_mk = Vec::new();
...
fn intercepted_authenticate(
    ...
    let client_kem_pk = client_state.pq_ephemeral_public_key.clone();
    let client_kem_sk = client_state.pq_ephemeral_secret_key.clone();
    ...
    let server_kem_ss = server_state.pq_shared_secret.clone();
    let credential_response = ke2.credential_response;
    let kem_ciphertext = ke2.kem_ciphertext.clone();
...
fn adversary_derive_session_key(
    ...
    let mut session_key = vec![0u8; HASH_LENGTH];
...
mod p2_password_secrecy {
    ...
    fn record_reveals_no_password_material() {
        ...
        let records: Vec<Vec<u8>> = passwords
            .iter()
...
mod p5_mutual_authentication {
    ...
    fn client_rejects_tampered_ke2_mac() {
        ...
        let mut ke2 = Ke2Message::new();
        generate_ke2(
            &responder, &ke1_bytes, ACCOUNT_ID, &credentials, &mut ke2, &mut server_state,
        )
        .unwrap();
    ...
    fn replay_ke2_from_different_session_fails() {
        ...
        generate_ke2(&responder, &ke1_bytes_1, ACCOUNT_ID, &creds, &mut ke2_1, &mut ss1).unwrap();
...
mod p7_offline_dictionary_resistance {
    ...
    fn envelope_not_decryptable_without_password() {
        ...
        let tag: &[u8; SECRETBOX_MAC_LENGTH] = envelope_data[NONCE_LENGTH + ct_size..]
            .try_into()
            .unwrap();
    ...
    fn wrong_password_dictionary_attack_fails() {
        ...
        for wrong_pwd in &dictionary {
            ...
            let mut ke2 = Ke2Message::new();
            generate_ke2(&responder, &ke1_bytes, ACCOUNT_ID, &creds, &mut ke2, &mut ss).unwrap();
