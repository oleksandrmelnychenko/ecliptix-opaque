    ...
    group.bench_function("finalize", |b| {
        b.iter_batched(
            || {
                ...
                create_registration_response(&responder, &r.data, ACCOUNT_ID, &mut rsp).unwrap();
                (s, rsp)
            },
            |(mut s, rsp)| {
                let mut record = RegistrationRecord::new();
                finalize_registration(&initiator, &rsp.data, &mut s, &mut record).unwrap();
            },
    ...
    group.bench_function("generate_ke3", |b| {
        b.iter_batched(
            || {
                ...
                generate_ke2(&responder, &k1b, ACCOUNT_ID, &credentials, &mut k2, &mut ss)
                    .unwrap();
...
fn bench_full_authentication(c: &mut Criterion) {
    ...
    group.bench_function("authentication_e2e", |b| {
        b.iter(|| {
            ...
            let mut server_sk = Vec::new();
            let mut server_mk = Vec::new();
            responder_finish(&k3b, &mut ss, &mut server_sk, &mut server_mk).unwrap();

            let mut client_sk = Vec::new();
            let mut client_mk = Vec::new();
